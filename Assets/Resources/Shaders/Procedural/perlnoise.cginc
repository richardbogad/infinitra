...
float mysnoise(float3 p_in, float3 Offset)
{
	int3 O = floor(Offset);
	float3 p = p_in + (Offset - O);
	int3 P = floor(p);
	p -= P;
	P = (P+O) & int3(255,255,255);
...
float4 mydsnoise(float3 p_in, float3 Offset)
{
	int3 O = floor(Offset);
	float3 p = p_in + (Offset - O);
	int3 P = floor(p);
...
float mynoise(float3 p, float3 Offset)
{
	return 0.5*mysnoise(p, Offset)+0.5;
}

float4 mydnoise(float3 p, float3 Offset)
{
	return 0.5*mydsnoise(p, Offset)+float4(0.5,0,0,0);
}

void basicnoise_float(float3 p, float3 Offset, Texture2D perm_tex_in, Texture2D grad_tex_in, out float Out)
{
	perm_tex = perm_tex_in;
	grad_tex = grad_tex_in;

	Out = mynoise(p, Offset);
}

void dbasicnoise_float(float3 p, float3 Offset, Texture2D perm_tex_in, Texture2D grad_tex_in, out float4 Out)
{
	perm_tex = perm_tex_in;
	grad_tex = grad_tex_in;

	Out = mydnoise(p, Offset);
}
...
float filteredsnoise(float3 p, float3 Offset, float width)
{
	return mysnoise(p, Offset) * (1 - smoothstep(0.25,0.75,width));
}

float4 filtereddsnoise(float3 p, float3 Offset, float width)
{
	return mydsnoise(p, Offset) * (1 - smoothstep(0.25,0.75,width));
}
...
void fractalsum_float(float3 P, float3 Offset, float fPixSize_in, Texture2D perm_tex_in, Texture2D grad_tex_in, out float Out)
{
	float fracsum = 0;
	...
	while(fw < 1.0)
	{
		fracsum += (1/freq) * filteredsnoise(P*freq, Offset*freq, fw);
		freq*=2; fw *= 2;
	}
#else
   ...
   if (scale > fPixSize)
   {
		...
		fracsum += weight * scale * mysnoise(P/scale, Offset/scale);
   }
#endif
...
void dfractalsum_float(float3 P, float3 Offset, float fPixSize_in, Texture2D perm_tex_in, Texture2D grad_tex_in, out float4 Out)
{
	float4 fracsum = 0;
	float fPixSize = VerifyPixSize(fPixSize_in);
	...
	while(fw < 1.0)
	{
		fracsum += float4((1/freq),1,1,1) * filtereddsnoise(P*freq, Offset*freq, fw);
		freq*=2; fw *= 2;
	}
#else
	const float twice = 2*fPixSize;
	float scale = 1;
   	for(scale = 1; scale > twice; scale /= 2) 
    		fracsum += float4(scale,1,1,1) * mydsnoise(P/scale, Offset/scale);
   ...
   if (scale > fPixSize)
   {
		float weight = (scale / fPixSize) - 1;
		weight = clamp(weight, 0, 1);
		fracsum += weight * float4(scale,1,1,1) * mydsnoise(P/scale, Offset/scale);
   }
#endif
...
void turbulence_float(float3 P, float3 Offset, float fPixSize_in, Texture2D perm_tex_in, Texture2D grad_tex_in, out float Out)
{
	float turbulence = 0;
	float fPixSize = VerifyPixSize(fPixSize_in);
	...
	while(fw < 1.0)
	{
		turbulence += (1/freq) * filteredabs( filteredsnoise(P*freq, Offset*freq, fw), 2*fw);
		freq*=2; fw *= 2;
	}
#else
   	const float twice = 2*fPixSize;
	float scale = 1;
   	for(scale = 1; scale > twice; scale /= 2) 
    		turbulence += scale * abs(mysnoise(P/scale, Offset/scale));
   ...
   if (scale > fPixSize)
   {
		float weight = (scale / fPixSize) - 1;
		weight = clamp(weight, 0, 1);
		turbulence += weight * scale * abs(mysnoise(P/scale, Offset/scale));
   }
#endif
...
void fractalsummax_float(float3 P, float3 Offset, float fPixSize_in, int iMaxOctaves_in, Texture2D perm_tex_in, Texture2D grad_tex_in, out float Out)
{
   	float fracsum = 0;
	float fPixSize = VerifyPixSize(fPixSize_in);
	...
	if(iPermOctaves>0)
	{
		...
		{
			fracsum += ((1-weight) / freq) * filteredsnoise(P*freq, Offset*freq, fw);
		}
	
#else
		const int iIterations = min(iPermOctaves-1, iMaxOctaves-2);
		fracsum += (weight / freq) * mysnoise(P*freq, Offset*freq);
		for(int k=0; k<iIterations; k++)
		{
			freq /= 2;
		...
		{
			freq /= 2;
			fracsum += ((1-weight) / freq) * mysnoise(P*freq, Offset*freq);
		}
		//assert(scale>=1);
#endif
	}
...
void dfractalsummax_float(float3 P, float3 Offset, float fPixSize_in, int iMaxOctaves_in, Texture2D perm_tex_in, Texture2D grad_tex_in, out float4 Out)
{
   	float4 fracsum = 0;
	float fPixSize = VerifyPixSize(fPixSize_in);
	...
	if(iPermOctaves>0)
	{
		...
		for(int k=0; k<iIterations; k++)
		{
			fracsum += float4(1.0/freq,1,1,1) * filtereddsnoise(P*freq, Offset*freq, fw);
			freq/=2; fw /= 2;
		}
		if(iPermOctaves>=iMaxOctaves)
		{
			fracsum += (1-weight) * float4(1.0/freq,1,1,1) * filtereddsnoise(P*freq, Offset*freq, fw);
		}
	
#else
		const int iIterations = min(iPermOctaves-1, iMaxOctaves-2);
		fracsum += weight * float4(1.0/freq,1,1,1) * mydsnoise(P*freq, Offset*freq);
		for(int k=0; k<iIterations; k++)
		{
			freq /= 2;
			fracsum += float4(1.0/freq,1,1,1) * mydsnoise(P*freq, Offset*freq);
		}
		
		if(iPermOctaves>=iMaxOctaves)
		{
			freq /= 2;
			fracsum += (1-weight) * float4(1.0/freq,1,1,1) * mydsnoise(P*freq, Offset*freq);
		}
		//assert(scale>=1);
#endif
	}
...
void turbulencemax_float(float3 P, float3 Offset, float fPixSize_in, int iMaxOctaves_in, Texture2D perm_tex_in, Texture2D grad_tex_in, out float Out)
{
   	float turbulence = 0;
	float fPixSize = VerifyPixSize(fPixSize_in);
	...
	if(iPermOctaves>0)
	{
		...
		if(iPermOctaves>=iMaxOctaves)
		{
			turbulence += ((1-weight) / freq) * filteredabs( filteredsnoise(P*freq, Offset*freq, fw), 2*fw);
		}
#else
		const int iIterations = min(iPermOctaves-1, iMaxOctaves-2);
		turbulence += (weight / freq) * abs(mysnoise(P*freq, Offset*freq));
		for(int k=0; k<iIterations; k++)
		{
			freq /= 2;
			turbulence += abs(mysnoise(P*freq, Offset*freq)) / freq;
		}
		
		if(iPermOctaves>=iMaxOctaves)
		{
			freq /= 2;
			turbulence += ((1-weight) / freq) * abs(mysnoise(P*freq, Offset*freq));
		}
		//assert(scale>=1);
#endif
	}
